...
lib = function() {
  ...
  return {
     ...
     * @type {String}
     */
    personalEmail: "jkarasavvaidis@gmail.com",
     ...
     * @type {String}
     */
    websiteEmail: "webContactForm@studio110.eu",
    ...
    waitForElement: function(elementPath, callBack) {
      if (typeof elementPath !== 'string') {
        this.log('lib : ERROR : waitForElement : elementPath is not a string');
        return;
      }
      var id = (elementPath.indexOf('#') > -1) ? true : false,
        element = (id) ?
        document.getElementById(elementPath.substring(1)) :
        document.getElementsByClassName(elementPath.substring(1))[0];

      if (typeof callBack !== 'function') {
        this.log('lib : ERROR : waitForElement : callBack is not a function');
        return;
      }
      if (!element) {
        this.log('lib : ERROR : waitForElement : element doesn\'t exist');
        return;
      }
